...
class Place(object):
    ...
    def add_insect(self, insect):
        ...
        """
        if insect.is_ant:
            print("DEBUG:", self.ant, insect)
            if self.ant is None:
                self.ant = insect
                insect.place = self
                print("DEBUG:", self.ant, insect.place)
            else:
                # BEGIN Problem 9
                if self.ant.can_contain(insect):
                    ...
                    print("DEBUG:", self.ant.can_contain(insect), insect.can_contain(self.ant))
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        checked = 0
        while checking_place.entrance is not hive and not checking_place.bees or checked < self.min_range:
            checking_place = checking_place.entrance
            checked += 1
        if checked <= self.max_range:
...
class NinjaAnt(Ant):
    ...
    def action(self, colony):
        ...
        "*** YOUR CODE HERE ***"
        bees_copy = self.place.bees[:]
        [Insect.reduce_armor(b, self.damage) for b in bees_copy]
        # END Problem 7
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        if not self.real:
            ...
            checked_place = self.place.exit
            while checked_place:
                checked_ant = checked_place.ant
                if checked_ant:
                    if checked_ant not in self.buffed_ants:
                        checked_ant.damage *= 2
                        self.buffed_ants.append(checked_ant)
                    if checked_ant.is_container and checked_ant.contained_ant:
                        inside_ant = checked_ant.contained_ant
                        if inside_ant not in self.buffed_ants:
                            inside_ant.damage *= 2
                            self.buffed_ants.append(inside_ant)
                checked_place = checked_place.exit
        "*** YOUR CODE HERE ***"
        # END Problem 13
...
def make_slow(action, bee):
    ...
    def new_action(colony):
        ...
        if not colony.time % 2:
            action(colony)
            print("DEBUG:", 'test', bee.place.name, colony.time)
        print("DEBUG:", "end", bee.timer, bee.action)  
    return new_action
    "*** YOUR CODE HERE ***"
    # END Problem EC
...
def make_scare(action, bee):
    ...
    def new_action(colony):
        print("DEBUG:", "Scare timer", bee.timer[i])
        if bee.timer[i]:
            bee.timer[i] -= 1
        else:
            bee.scared = False
        action(colony)
        print("DEBUG:", bee.place.name, bee.scared)
    return new_action 
    "*** YOUR CODE HERE ***"
    # END Problem EC
...
def apply_effect(effect, bee, duration):
    ...
    # BEGIN Problem EC
    bee.timer.append(duration)
    bee.action = effect(bee.action, bee)
    print("DEBUG:", bee.action)
    "*** YOUR CODE HERE ***"
    # END Problem EC
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self, hive):
        # BEGIN Problem OPTIONAL
        dict_insects = {}
        checked_place = self.place
        places_away = 0
        while not checked_place == hive:
            if checked_place.ant and not checked_place.ant == self:
                dict_insects[checked_place.ant] = places_away
            for b in checked_place.bees:
                dict_insects[b] = places_away
            places_away += 1
            checked_place = checked_place.entrance
        return dict_insects
        # END Problem OPTIONAL

    def calculate_damage(self, distance):
        # BEGIN Problem OPTIONAL
        print("DEBUG:", self.insects_shot, distance)
        result = 2 - 0.2 * distance - 0.05 * self.insects_shot
        if result > 0:
            return result
        return 0
        # END Problem OPTIONAL
